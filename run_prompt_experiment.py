...
from transformers import AutoModelForImageTextToText, AutoProcessor, LlavaOnevisionForConditionalGeneration
...
def main():
    ...
    print(f"Model: {args.model} ({'llava-onevision' if is_llava else 'qwen'})")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc=f"{args.prompt_mode}"):
        ...
        # Limit image size to avoid OOM on smaller GPUs
        max_side = 672
